...
//随机文字烟花内容
const randomWords = ["小秦同学，新年快乐！遇见你很高兴！"];
const wordDotsMap = {};
// 延迟初始化，等待字体加载完成后再生成点阵
// 将在 preloadFonts 完成后初始化
...
function preloadFonts() {
	return new Promise((resolve) => {
		...
		testText.style.fontFamily = 'Gabriola, 华文琥珀, serif';
		testText.style.fontSize = '1px';
		testText.style.position = 'absolute';
		testText.style.left = '-9999px';
		testText.textContent = '测试';
		document.body.appendChild(testText);
		
		// 使用 document.fonts.ready 检查字体加载状态
		if (document.fonts && document.fonts.ready) {
			document.fonts.ready.then(() => {
				document.body.removeChild(testText);
				// 字体加载完成后初始化 wordDotsMap
			...
			// 对于不支持 Font Loading API 的浏览器，延迟一段时间
			setTimeout(() => {
				document.body.removeChild(testText);
				// 字体加载完成后初始化 wordDotsMap
				randomWords.forEach((word) => {
...
if (IS_HEADER) {
	...
	setTimeout(() => {
		// 加载 soundManager 和字体
		var promises = [soundManager.preload(), preloadFonts()];

		// 在 soundManager 和字体加载完毕后调用 init
...
soundBtn.addEventListener('click', () => {
    toggleSound(); // 调用现有 toggleSound 函数切换状态
});

// 在 script.js 中添加
document.addEventListener('click', () => {
    if (store.state.soundEnabled && audio.paused) {
        audio.play().catch(e => console.log('用户交互后播放失败:', e));
    }
}, { once: true }); // 只触发一次